...
void change_yb_task(intptr_t unused) {
	...
	if(container[counter] == 4){
		...
		ev3_motor_reset_counts(EV3_PORT_D);
		do{
		}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -375 * a);
	...
	switch (counter) {
		...
		case 3:
			...
			if (counter5 == 0){
				...
				ev3_motor_reset_counts(EV3_PORT_D);
				do{
				}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -315 * a);
				ev3_stp_cyc(TRACE);
			}
			counter5 = 0;
...
void main_task(intptr_t unused) {
	...
	//ここに角度で進むを入れるとうまくいくかも　いつも惰性で進んでいるように見え
	...
	for (i = 1; i <= 5; i++) {
		...
		*pHT = 0;
		ev3_sta_cyc(HT4);
		do{
			if((*pHT == 7 || *pHT == 8 || *pHT == 9) && ship[0] == 0 && twice == 0) {
				ev3_speaker_play_tone(1975.53, 200);
				ship[0] = i;
				ship2[j] = 1;
				j += 1;
