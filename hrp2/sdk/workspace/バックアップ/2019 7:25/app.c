...
void ht4color_task(intptr_t unused) {
	...
	if(choice == 1){//赤
		if(rgb4.b > 0 && rgb4.b <= 170){//230
			wup_tsk(MAIN_TASK);
		}
	}else if(choice == 2){//緑
		if(rgb4.g > 0 && rgb4.g <= 170){//r
			wup_tsk(MAIN_TASK);
		}
	}else if(choice == 3){//黄
		if(rgb4.b > 0 && rgb4.b <= 170){//170//230
			wup_tsk(MAIN_TASK);
		}
	}else if(choice == 4){//青
		if(rgb4.g > 0 && rgb4.g <= 160){//170
			wup_tsk(MAIN_TASK);
		}
	}
...
void drop_task(intptr_t unused) {
	if(direction == 1){
		...
		*/
		...
		ev3_motor_set_power(EV3_PORT_C, 80);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_C) <= 350);//315


		//ev3_motor_set_power(EV3_PORT_C, 20);
		ev3_motor_rotate(EV3_PORT_C, 270, 20, false);//300
...
void main_task(intptr_t unused) {
	...
	*/
	...
	ev3_motor_rotate(EV3_PORT_C, -160, 30, false);//190
	...
	*/
	...
	do{
	}while(ev3_motor_get_counts(EV3_PORT_B) <= 1030);//1000
	//ev3_motor_stop(EV3_PORT_B, true);
	...
	*/
	...
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 520);//600//400//340
	...
	*/
	...
	if(device[1] == 0){
		...
		ev3_motor_reset_counts(EV3_PORT_D);
		do{
		}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 45);
	...
	power = slow;
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 145);//135
