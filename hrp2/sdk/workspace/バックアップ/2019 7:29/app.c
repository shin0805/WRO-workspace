	...
	*/
	...
	switch (curve) {
		...
		case 7://真ん中への右 (両方黒から)
			...
			ev3_motor_set_power(EV3_PORT_D, -20);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_D) * -1 <= 45);
			//do{
			//}while(ev3_color_sensor_get_reflect(EV3_PORT_3) >= threshold3);
...
void ht4color_task(intptr_t unused) {
	...
	if(choice == 1){//赤
		if(rgb4.b > 0 && rgb4.b <= 220){//230//170
			wup_tsk(MAIN_TASK);
		}
	}else if(choice == 2){//緑
		...
		if(rgb4.b > 0 && rgb4.b <= 230){//170//230
			wup_tsk(MAIN_TASK);
		}
	}else if(choice == 4){//青
...
void main_task(intptr_t unused) {
	...
	*/
	...
	ev3_motor_reset_counts(EV3_PORT_D);
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= angle23);//100
