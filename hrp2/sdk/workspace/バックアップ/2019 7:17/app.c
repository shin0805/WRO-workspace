...
void curve_task(intptr_t unused) {
	switch (curve) {
		case 1://真ん中への右
			...
			do{
			}while(ev3_motor_get_counts(EV3_PORT_D) * -1 <= 140);
...
void drop_task(intptr_t unused) {
	if(direction == 1){
		...
		*/
		...
		ev3_motor_rotate(EV3_PORT_C, 310, 20, false);//300
		...
		*/
		...
		ev3_sta_cyc(BACK);
		do{
		}while((ev3_motor_get_counts(EV3_PORT_D) - ev3_motor_get_counts(EV3_PORT_A)) / 2 <= 95);
		ev3_stp_cyc(BACK);
		...
		//ev3_motor_set_power(EV3_PORT_C, 20);
		ev3_motor_rotate(EV3_PORT_C, 310, 20, false);
		/*
		do{
		}while(ev3_motor_get_counts(EV3_PORT_C) <= 615);
		...
		*/
		...
		//リンクをたたみきる
		ev3_motor_rotate(EV3_PORT_C, 280, 20, false);
		//ev3_motor_set_power(EV3_PORT_C, 20);
		/*
		do{
		}while(ev3_motor_get_counts(EV3_PORT_C) <= 615);
...
void main_task(intptr_t unused) {
	...
	*/
	...
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) <= 155);
	...
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) <= 15);
	...
	ev3_motor_set_power(EV3_PORT_D, slows);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_D) * -1 <= 155);
	...
	ev3_motor_rotate(EV3_PORT_C, -190, 30, false);//190//265
	...
	if(h_rgb > 100){
		...
		ev3_motor_reset_counts(EV3_PORT_D);
		do{
		}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 180);
	...
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 300);//600
	...
	ev3_motor_set_power(EV3_PORT_D, slows);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_D) * -1 <= 155);//150//190
	...
	ev3_motor_set_power(EV3_PORT_D, slows);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) <= 25);//25//40
	...
	ev3_motor_set_power(EV3_PORT_A, slows * -1);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) <= 155);//150//150
