...
void curve_task(intptr_t unused) {
	switch (curve) {
		case 1://真ん中への右
			...
			do{
			}while(ev3_motor_get_counts(EV3_PORT_D) * -1 <= 270);

			ev3_motor_reset_counts(EV3_PORT_D);
			ev3_motor_set_power(EV3_PORT_A, -30);
			ev3_motor_set_power(EV3_PORT_D, -30);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_D) * -1 <= 80);
...
void drop_task(intptr_t unused) {
	...
	if(direction == 1){
		...
		*/
		...
		do{
		}while(ev3_motor_get_counts(EV3_PORT_B) * -1 <= 50);
		ev3_motor_stop(EV3_PORT_B, true);
		...
		ev3_motor_set_power(EV3_PORT_B, 50);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_B) <= 200);
		ev3_motor_stop(EV3_PORT_B, true);
		...
		*/
		...
		ev3_motor_set_power(EV3_PORT_B, 50);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_B) <= 50);
		ev3_motor_stop(EV3_PORT_B, true);
		...
		ev3_motor_set_power(EV3_PORT_B, -50);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_B) * -1 <= 200);
		ev3_motor_stop(EV3_PORT_B, true);
	...
	*/
	...
	if(h_rgb > 100){
		...
		do {
			...
			/*if(ev3_motor_get_counts(EV3_PORT_C) * -1 >= 50){
				while2 = 1;
			}else{
				ev3_motor_set_power(EV3_PORT_C, -30);
			}
