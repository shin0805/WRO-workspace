...
void right_task(intptr_t unused) {
	...
	*/
	...
	do{
		ev3_motor_set_power(EV3_PORT_A, pturn * -1 + 10);
		ev3_motor_set_power(EV3_PORT_D, pturn * -1 + 10);
	}while(ev3_motor_get_counts(EV3_PORT_A) >= -65);//70
	...
	*/
	...
	switch (curve) {
		...
		case 7://真ん中への右 (両方黒から)
			...
			ev3_motor_set_power(EV3_PORT_D, -30);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_D) * -1 <= 140);//170
			...
			ev3_motor_set_power(EV3_PORT_D, -20);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_D) * -1 <= 20);//30
			//do{
			//}while(ev3_color_sensor_get_reflect(EV3_PORT_3) >= threshold3);
		...
		case 8://真ん中への左 (両方黒から)
			...
			ev3_motor_set_power(EV3_PORT_D, 0);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) <= 140);//170
			...
			ev3_motor_set_power(EV3_PORT_D, 20);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) <= 20);//30
			//do{
			//}while(ev3_color_sensor_get_reflect(EV3_PORT_2) >= threshold2);
...
void ht4color_task(intptr_t unused) {
	...
	if(choice == 1){//赤
		if(rgb4.b > 0 && rgb4.b <= 220){//230//170//210
			wup_tsk(MAIN_TASK);
		}
	}else if(choice == 2){//緑
...
void drop_task(intptr_t unused) {
	if(direction == 1){
		...
		*/
		...
		do{
		}while(ev3_motor_get_counts(EV3_PORT_A) * -1 <= 30);//35
		ev3_motor_stop(EV3_PORT_A, true);
		ev3_motor_stop(EV3_PORT_D, true);
		...
		*/
		...
		ev3_motor_set_power(EV3_PORT_D, slow * -1);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_A) <= 30);//35
		...
		*/
		...
		do{
		}while(ev3_motor_get_counts(EV3_PORT_D) <= 30);//35
		ev3_motor_stop(EV3_PORT_D, true);
		ev3_motor_stop(EV3_PORT_A, true);
		...
		*/
		...
		ev3_motor_set_power(EV3_PORT_A, slow);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_D) * -1 <= 30);//35
...
void main_task(intptr_t unused) {
	...
	*/
	...
	ev3_motor_rotate(EV3_PORT_C, -185, 30, false);//190//160//170//180が落ち着いた頃//185は満タン時
	...
	*/
	...
	ev3_motor_set_power(EV3_PORT_D, -30);
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 140);//125//150//145
	...
	*/
	...
	ev3_sta_cyc(BACK);
	do{
	}while((ev3_motor_get_counts(EV3_PORT_D) - ev3_motor_get_counts(EV3_PORT_A)) / 2 <= 200);//150
	ev3_stp_cyc(BACK);
	...
	*/
	...
	ev3_motor_reset_counts(EV3_PORT_D);
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 1850);//1950
