...
void ht4color_task(intptr_t unused) {
	...
	if(choice == 1){
		...
		if(rgb4.g > 0 && rgb4.g <= 230){
			wup_tsk(MAIN_TASK);
		}
	}else if(choice == 3){
		if(rgb4.b > 0 && rgb4.b <= 170){
			wup_tsk(MAIN_TASK);
		}
	}
...
void main_task(intptr_t unused) {
	...
	*/
	...
	ev3_motor_set_power(EV3_PORT_A, slows * -1);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) <= 295);//290
	...
	*/
	...
	ev3_motor_rotate(EV3_PORT_B, -840, 80, false);
	...
	if(h_rgb > 100){
		...
		*/
		...
		ev3_motor_reset_counts(EV3_PORT_D);
		do{
		}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 200);//250
