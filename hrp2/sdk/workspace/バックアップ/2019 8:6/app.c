...
void ht4color_task(intptr_t unused) {
	...
	if(choice == 1){//赤
		if(rgb4.b > 0 && rgb4.b <= 210){//230//170//210
			wup_tsk(MAIN_TASK);
		}
	}else if(choice == 2){//緑
		if(rgb4.g > 0 && rgb4.g <= 220){//r//170//230//210
			wup_tsk(MAIN_TASK);
		}
	}else if(choice == 3){//黄
...
void drop_task(intptr_t unused) {
	if(direction == 1){
		...
		*/
		...
		//ev3_motor_set_power(EV3_PORT_C, 20);
		ev3_motor_rotate(EV3_PORT_C, 200, 20, false);//300//200
...
void main_task(intptr_t unused) {
	...
	*/
	...
	ev3_motor_reset_counts(EV3_PORT_D);
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 100);//100//90//85
