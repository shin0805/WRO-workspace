...
double two_slow = 1.04;//2.14
double one_fast = 3.0;//4.0
double one_slow = 1.7;//1.9
...
void curve_task(intptr_t unused) {
	switch (curve) {
		...
		case 7://真ん中への右 (両方黒から)
			...
			ev3_motor_set_power(EV3_PORT_D, -30);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_D) * -1 <= 85);
...
void rotation_task(intptr_t unused) {
	...
	ev3_motor_reset_counts(EV3_PORT_A);
	do{
		ev3_motor_set_power(EV3_PORT_A, -30);//pturn * -1 + 10);//30
		ev3_motor_set_power(EV3_PORT_D, -30);//pturn * -1 + 10);
...
void drop_task(intptr_t unused) {
	if(direction == 1){
		...
		*/
		...
		ev3_motor_rotate(EV3_PORT_C, 300, 20, false);//300
...
void main_task(intptr_t unused) {
	...
	ev3_motor_set_power(EV3_PORT_D, slows);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_D) * -1 <= 160);
	...
	if(h_rgb > 100){
		...
		ev3_motor_set_power(EV3_PORT_A, slow);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_A) * -1 <= 350);
		...
		*/
		...
		ev3_motor_reset_counts(EV3_PORT_D);
		do{
		}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 220);//250
		...
		do {
			...
			if(direction == 1){
				//左向きに回す
				if(ev3_motor_get_counts(EV3_PORT_B) * -1 >= 150){
					ev3_motor_stop(EV3_PORT_B, true);
					while1 = 1;
				}
			}else if(direction == 2){
				...
				//右向きに回す
				if(ev3_motor_get_counts(EV3_PORT_B) >= 150){
					ev3_motor_stop(EV3_PORT_B, true);
					while1 = 1;
				}
			}else if(direction == 4){
				//後ろ向きに回す
				if(ev3_motor_get_counts(EV3_PORT_B) >= 300){
					ev3_motor_stop(EV3_PORT_B, true);
					while1 = 1;
				}
			}
			...
			//while2
			//リンクを前に出す
			//if(ev3_motor_get_counts(EV3_PORT_C) * -1 >= 150){
				//ev3_motor_stop(EV3_PORT_C, true);
				//while2 = 1;
			//}

			if(ev3_motor_get_counts(EV3_PORT_C) * -1 >= 50 && while2 == 0){
				while2 = 1;
	...
	ev3_motor_set_power(EV3_PORT_D, slows);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_D) * -1 <= 190);//150
	...
	ev3_motor_set_power(EV3_PORT_D, slows);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) <= 40);//25
	...
	ev3_motor_set_power(EV3_PORT_A, slows * -1);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) <= 190);//150
	...
	if(device[1] == 0){
		...
		ev3_motor_set_power(EV3_PORT_D, slow * -1);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_D) <= 350);
	...
	ev3_motor_reset_counts(EV3_PORT_D);
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 90);//135
	...
	ev3_motor_reset_counts(EV3_PORT_D);
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 1360);
	...
	ev3_motor_set_power(EV3_PORT_D, slow);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_D) * -1 <= 350);
