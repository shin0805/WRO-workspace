...
void drop_task(intptr_t unused) {
	...
	if(direction == 1){
		...
		*/
		...
		ev3_motor_set_power(EV3_PORT_C, 60);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_C) <= 60);
		...
		ev3_motor_set_power(EV3_PORT_C, 60);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_C) <= 300);
...
void main_task(intptr_t unused) {
	...
	*/
	...
	ev3_motor_set_power(EV3_PORT_D, slows);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_D) * -1 <= 285);
	...
	if(h_rgb > 100){
		...
		/*
		ev3_motor_rotate(EV3_PORT_C, -170, 10, false);
	...
	*/
	...
	if(device[1] == 0){
		...
		for (k = 0; k < 2; k++) {
			...
			ev3_sta_cyc(FOWARD);
			do{
			}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 85);
			ev3_stp_cyc(FOWARD);
		}
	...
	*/
	...
	power = -20;
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 330);
