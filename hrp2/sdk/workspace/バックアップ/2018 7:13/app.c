...
void change_yb_task(intptr_t unused) {
	...
	if(container[counter] == 4){
		...
		do{
		}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 85 * a);
	}
...
void ship_right_task(intptr_t unused) {
	...
	angle = 45 + (6 - ship[container[counter] - 1]) * 249 * a;
...
void main_task(intptr_t unused) {
	...
	if(container[0] != 0) {
		...
		if(container[3] == 3 || container[3] == 4) {
			...
			//軸に乗せる
			ev3_motor_rotate(EV3_PORT_A, -75 * a, slow * -1, false);
			ev3_motor_rotate(EV3_PORT_D, -75 * a, slow * -1, true);
	...
	for (i = 1; i <= 5; i++) {
		...
		*pHT = 0;
		...
		if (ship[container[counter_a] - 1] == i) {
			...
			//方向転換
			ev3_motor_rotate(EV3_PORT_A, 290 * a, 30, true);
	...
	*pHT = 0;
		...
		switch (counter_c) {
			case 1:
				...
				if(container[1] == 1 || container[1] == 2) {
					...
					if(container[2] == 0){
						...
						ev3_motor_reset_counts(EV3_PORT_A);
						do{
						}while(ev3_motor_get_counts(EV3_PORT_A) >= -600 * a);
						ev3_stp_cyc(TRACE);
						...
						ev3_motor_set_power(EV3_PORT_D, fast);
						do{
						}while(ev3_motor_get_counts(EV3_PORT_D) >= -165 * a);
			...
			case 2:
				...
				if(container[2] == 3 || container[2] == 4) {
					...
					do{
					}while(ev3_color_sensor_get_reflect(EV3_PORT_3) >=70 || ev3_color_sensor_get_reflect(EV3_PORT_2) >=20);
					ev3_speaker_play_tone(1174.66, 200);
					ev3_stp_cyc(TRACE3);
	...
	//ゴールエリアにinする
	ev3_motor_rotate(EV3_PORT_A, -285 * a, slow * -1, false);
	ev3_motor_rotate(EV3_PORT_D, -285 * a, slow * -1, true);
