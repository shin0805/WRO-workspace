...
void change_yb_task(intptr_t unused) {
	...
	ev3_motor_rotate(EV3_PORT_A, -175 * a, 20, false);
	ev3_motor_rotate(EV3_PORT_D, -175 * a, 20, true);
...
void ship_right_task(intptr_t unused) {
	...
	angle = 45 + (6 - ship[container[counter] - 1]) * 246 * a;
...
void main_task(intptr_t unused) {
	...
	for (i = 1; i <= 5; i++) {
		...
		*pHT = 0;
		...
		if (ship[container[counter_a] - 1] == i) {
			...
			//方向転換
			ev3_motor_rotate(EV3_PORT_A, 280 * a, 30, true);
	...
	*pHT = 0;
	...
	if (ship[container[counter_a] - 1] == 6) {
		...
		//進む
		ev3_motor_rotate(EV3_PORT_A, -130 * a, 30, false);
		ev3_motor_rotate(EV3_PORT_D, -130 * a, 30, true);
	...
	for (k = 0; k < 2 ; k++) {
		...
		if (k == 0) {
			...
			if (container[1] == 0) {
				...
				//壁合わせ and 方向転換　(左へ)
				ev3_motor_reset_counts(EV3_PORT_A);
				ev3_motor_set_power(EV3_PORT_A, fast * -1 + 5);
		...
		switch (counter_c) {
			...
			case 3:
				...
				ev3_motor_reset_counts(EV3_PORT_A);
				do{
				}while(ev3_motor_get_counts(EV3_PORT_A) >= -325 * a);
	...
	//ゴールエリアにinする
	ev3_motor_rotate(EV3_PORT_A, -295 * a, slow * -1, false);
	ev3_motor_rotate(EV3_PORT_D, -295 * a, slow * -1, true);
