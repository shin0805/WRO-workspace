...
void change_yb_task(intptr_t unused) {
	...
	if(container[counter] == 4){
		...
		ev3_motor_reset_counts(EV3_PORT_D);
		do{
		}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -380 * a);
...
void ship_left_task(intptr_t unused) {
	...
	angle = 45 + (ship[container[counter] - 1] - 1) * 250 * a;
...
void ship_right_task(intptr_t unused) {
	...
	angle = 45 + (6 - ship[container[counter] - 1]) * 250 * a;
...
void main_task(intptr_t unused) {
	...
	for (i = 1; i <= 5; i++) {
		...
		*pHT = 0;
		...
		if (ship[container[counter_a] - 1] == i) {
			...
			if (ev3_color_sensor_get_reflect(EV3_PORT_2) >= 40){
				ev3_speaker_play_tone(1174.66, 200);
				ev3_motor_rotate(EV3_PORT_A, -10 * a, 20, false);
				ev3_motor_rotate(EV3_PORT_D, 10 * a, 20, true);
			...
			ev3_motor_rotate(EV3_PORT_A, 35 * a, 30, false);
			ev3_motor_rotate(EV3_PORT_D, 35 * a, 30, true);

			ev3_motor_stop(EV3_PORT_D, true);
			tslp_tsk(200);

			//方向転換
			ev3_motor_rotate(EV3_PORT_A, 315 * a, 30, true);
	...
	*pHT = 0;
		...
		if (k == 0) {
			...
			if (container[1] == 0) {
		  	...
		  	counter_c = 1;
				...
				//右端のshipをよけない方向転換
				//少し下がる
				ev3_motor_rotate(EV3_PORT_A, 92 * a, 30, false);
				ev3_motor_rotate(EV3_PORT_D, 92 * a, 30, true);
	...
	//ゴールエリアにinする
	ev3_motor_rotate(EV3_PORT_A, -305 * a, slow * -1, false);
	ev3_motor_rotate(EV3_PORT_D, -305 * a, slow * -1, true);
