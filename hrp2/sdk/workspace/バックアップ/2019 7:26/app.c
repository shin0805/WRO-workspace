...
void curve_task(intptr_t unused) {
	switch (curve) {
		...
		case 3://内側への右
			...
			ev3_motor_set_power(EV3_PORT_D, -30);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_D) * -1 <= 160);//190
		...
		case 4://内側への左
			...
			ev3_motor_set_power(EV3_PORT_D, 0);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) <= 160);//190

			ev3_motor_reset_counts(EV3_PORT_A);
			ev3_motor_set_power(EV3_PORT_A, 30);
			ev3_motor_set_power(EV3_PORT_D, 30);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) <= 120);//110
...
void drop_task(intptr_t unused) {
	if(direction == 1){
		...
		*/
		...
		do{
		}while(ev3_motor_get_counts(EV3_PORT_D) <= 40);
		ev3_motor_stop(EV3_PORT_D, true);
		ev3_motor_stop(EV3_PORT_A, true);
		...
		*/
		...
		ev3_motor_set_power(EV3_PORT_A, slow);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_D) * -1 <= 40);
...
void main_task(intptr_t unused) {
	...
	*/
	...
	power = slow;
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 120);//135//145
