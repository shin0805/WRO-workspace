...
void ht4color_task(intptr_t unused) {
	...
	if(choice == 1){//赤
		if(rgb4.b > 0 && rgb4.b <= 170){//230//170//210
			wup_tsk(MAIN_TASK);
		}
	}else if(choice == 2){//緑
...
void main_task(intptr_t unused) {
	...
	*/
	...
	//リンクをたたむ
	ev3_motor_rotate(EV3_PORT_C, 190, 20, false);//180
	...
	*/
	...
	if(device[0] == 2){
		...
		*/
		...
		do{
		}while(ev3_motor_get_counts(EV3_PORT_D) <= 470);

		ev3_motor_set_power(EV3_PORT_D, slow * -1);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_D) <= 520);
	...
	*/
	...
	ev3_motor_reset_counts(EV3_PORT_D);
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 90);//100//90//85
