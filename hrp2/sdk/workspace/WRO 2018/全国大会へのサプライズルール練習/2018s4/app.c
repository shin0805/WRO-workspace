...
//container[3]が1回目か最後かの変数 container[0]にも使う
int counter5 = 0;
	...
	switch (counter) {
		...
		case 3:
			...
			if (counter5 == 0){
				...
				power = slow;
				do{
				}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -320 * a);
				ev3_stp_cyc(TRACE);
			}
			counter5 = 0;
...
void main_task(intptr_t unused) {
	...
	*pHT = 0;
	...
	}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) * -1) / 2 <= 30 * a);
	...
	*pHT = 0;
	...
	yes_container[0] = no_color;
	yes_container[1] = no_color;
	yes_container[2] = no_color;
	yes_container[3] = no_color;
