...
void catch_task(intptr_t unused) {
	...
	if(counter == 0 || counter == 2) {
		...
		do{
		}while(ev3_motor_get_counts(EV3_PORT_A) <= 255 * a);
...
void main_task(intptr_t unused) {
			...
			if (counter_c == 1) {
				...
				if (counter_b == 1) {
					...
					if(angle >= 350 * a) {
						...
						ev3_motor_set_power(EV3_PORT_D, fast);
						do{
						}while(ev3_motor_get_counts(EV3_PORT_A) >= (angle - 250 * a) * -1);
					}
	...
	if (counter_b == 1) {
		...
		if (no_color <= 2) {
			...
			ev3_motor_set_power(EV3_PORT_D, -10);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) <= anfw * -1 * a);
			...
			if(angle >= 350 * a) {
				...
				ev3_motor_set_power(EV3_PORT_D, fast);
				do{
				}while(ev3_motor_get_counts(EV3_PORT_A) * -1 >= (angle - 250 * a) * -1);
			}
