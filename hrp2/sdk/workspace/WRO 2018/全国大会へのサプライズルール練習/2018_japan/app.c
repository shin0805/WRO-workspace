...
void change_yb_task(intptr_t unused) {
	...
	if(container[counter] == 4){
		...
		power = slow;
		do{
		}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -400 * a);
	...
	ev3_motor_rotate(EV3_PORT_A, 160 * a, slow * -1, false);
	ev3_motor_rotate(EV3_PORT_D, -160 * a, slow * -1, true);
...
void ship_left_task(intptr_t unused) {
	...
	//軸に乗せる
	ev3_motor_rotate(EV3_PORT_A, 60 * a, slow * -1, false);
	ev3_motor_rotate(EV3_PORT_D, -60 * a, slow * -1, true);
	...
	switch (counter) {
		case 0:
			if (counter5 == 0){
				...
				power = slow;
				do{
				}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -440 * a);
				ev3_stp_cyc(TRACE);
				...
				ev3_motor_rotate(EV3_PORT_A, turn * -1 * a, 30, false);
				ev3_motor_rotate(EV3_PORT_D, turn * -1 * a, 30, true);
		...
		case 1:
			...
			ev3_motor_set_power(EV3_PORT_D, 30);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) >= -15 * a);
...
void main_task(intptr_t unused) {
	...
	if(container[0] != 0) {
		...
		if(container[3] == 3 || container[3] == 4) {
			...
			if(yes_container[0] == 0){
				...
				ev3_motor_reset_counts(EV3_PORT_A);
				do{
				}while(ev3_motor_get_counts(EV3_PORT_A) <= 310 * a);
			...
			//contaienr[3]が赤、緑の場合
			ev3_motor_rotate(EV3_PORT_A, -190 * a, slow * -1, false);
			ev3_motor_rotate(EV3_PORT_D, 190 * a, slow * -1, true);
	...
	do{
		...
		*pHT = 0;
		...
		if (ship[container[counter_a] - 1] == ship[i]) {
			...
			if(ship[container[counter_a] - 1] == 1){
				ev3_stp_cyc(TRACE33);
				...
				ev3_motor_set_power(EV3_PORT_D, -10);
				do{
				}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) * -1) / 2 <= 55);

			}else{
				angle2 = (ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) * -1) / 2 + 105;

				ev3_motor_reset_counts(EV3_PORT_A);
				ev3_motor_reset_counts(EV3_PORT_D);
				do{
				}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) * -1) / 2 <= 105);
			...
			if(ship[container[counter_a] - 1] == 1){
				...
				ev3_motor_stop(EV3_PORT_C, true);
				tslp_tsk(200);
			}else if(ship[container[counter_a] - 1] == 6){
				//進む　壁回避
				ev3_motor_reset_counts(EV3_PORT_A);
				ev3_motor_set_power(EV3_PORT_A, slow * -1);
				ev3_motor_set_power(EV3_PORT_D, slow - 5);
			...
			if(ship[container[counter_a] - 1] == 3){
				//方向転換
				ev3_motor_rotate(EV3_PORT_A, turn, slow * -1, false);
				ev3_motor_rotate(EV3_PORT_D, turn, slow * -1, true);
	...
	for (k = 0; k < 2 ; k++) {
		...
		if (k == 0) {
			...
			if (container[1] == 0) {
				...
				ev3_motor_set_power(EV3_PORT_D, slow * -1);
				do{
				}while(ev3_motor_get_counts(EV3_PORT_D) <= 5 * a);
		  	...
		  	counter_c = 1;
				...
				if(1200 * a - angle >= 350 * a) {
					...
					ev3_motor_set_power(EV3_PORT_D, fast);
					do{
					}while(ev3_motor_get_counts(EV3_PORT_A) * -1 >= (1200 * a - angle - 250 * a) * -1);
				}

				ev3_motor_set_power(EV3_PORT_A, slow * -1);
				ev3_motor_set_power(EV3_PORT_D, slow);
				do{
				}while(ev3_motor_get_counts(EV3_PORT_A) * -1 >= (1200 * a - angle) * -1);
			...
			if (counter_c == 1) {
				...
				if (counter_b == 1) {
					...
					if(1200 * a - angle >= 350 * a) {
						...
						ev3_motor_set_power(EV3_PORT_D, fast * -1);
						do{
						}while(ev3_motor_get_counts(EV3_PORT_A) * -1 <= (1200 * a - angle - 250 * a));
					}

					ev3_motor_set_power(EV3_PORT_A, slow);
					ev3_motor_set_power(EV3_PORT_D, slow * -1);
					do{
					}while(ev3_motor_get_counts(EV3_PORT_A) * -1 <= (1200 * a - angle));
	...
	//ゴールエリアにinする
	if(no_color == 1 || no_color == 2){
		ev3_motor_rotate(EV3_PORT_A, 240 * a, slow * -1, false);
		ev3_motor_rotate(EV3_PORT_D, -240 * a, slow * -1, true);
		...
		ev3_motor_rotate(EV3_PORT_A, 240 * a, slow * -1 + 5, false);
		ev3_motor_rotate(EV3_PORT_D, -240 * a, slow * -1, true);
