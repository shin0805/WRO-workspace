...
int black = 25;
int white = 60;
int threshold = 45; //trace2とtrace22とtrace33は別規定
...
void change_yb_task(intptr_t unused) {
	...
	if(container[counter] == 4){
		...
		power = slow;
		do{
		}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -380 * a);
	...
	ev3_motor_rotate(EV3_PORT_A, 165 * a, slow * -1, false);
	...
	ev3_motor_rotate(EV3_PORT_A, -45 * a, 10, false);
	ev3_motor_rotate(EV3_PORT_D, 45 * a, 10, true);
...
void change_rg_task(intptr_t unused) {
	...
	ev3_motor_set_power(EV3_PORT_D, 40);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_D) <= 85 * a);

	ev3_motor_reset_counts(EV3_PORT_D);
	ev3_motor_set_power(EV3_PORT_A, 40);
	ev3_motor_set_power(EV3_PORT_D, 40);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_D) <= 120 * a);
...
void scan_task(intptr_t unused) {
	...
	if(counter == 3){
		...
		}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) * -1) / 2 <= 100 * a);

		if(separate == 1 && yes_container[counter] == 0) {
			yes_container[counter] = 2;
			ev3_speaker_play_tone(1396.91, 200);
			if(no_color != 2){
				container[counter] = 2;
...
void ship_left_task(intptr_t unused) {
	...
	//軸に乗せる
	ev3_motor_rotate(EV3_PORT_A, 65 * a, slow * -1, false);
	ev3_motor_rotate(EV3_PORT_D, -65 * a, slow * -1, true);
	...
	//方向転換
	ev3_motor_rotate(EV3_PORT_A, -turn * a, slow * -1, false);
	ev3_motor_rotate(EV3_PORT_D, -turn * a, slow * -1, true);
	...
	angle = 60 + (ship[container[counter] - 1] - 1) * 244 * a;

	if(ship[container[counter] - 1] == 1){
		ev3_motor_reset_counts(EV3_PORT_A);
		ev3_motor_reset_counts(EV3_PORT_D);
	...
	//方向転換
	ev3_motor_rotate(EV3_PORT_A, -turn, slow * -1, false);
	ev3_motor_rotate(EV3_PORT_D, -turn, slow * -1, true);
...
void ship_right_task(intptr_t unused) {
	...
	angle = 60 + (6 - ship[container[counter] - 1]) * 244 * a;
...
void catch_task(intptr_t unused) {
	switch (counter) {
		case 0:
			if (counter5 == 0){
				...
				ev3_motor_set_power(EV3_PORT_D, 30);
				do{
				}while(ev3_motor_get_counts(EV3_PORT_A) >= -75 * a);
				...
				ev3_motor_set_power(EV3_PORT_D, slow * -1);
				do{
				}while(ev3_motor_get_counts(EV3_PORT_D) <= 95 * a);
		...
		case 3:
			...
			if (counter5 == 0){
				...
				ev3_motor_reset_counts(EV3_PORT_D);
				do{
				}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -210 * a);
				ev3_stp_cyc(TRACE);
			}else{
				power = slows;
				ev3_motor_reset_counts(EV3_PORT_A);
				...
				power = slow;
				do{
				}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -325 * a);
				ev3_stp_cyc(TRACE);
			}
			counter5 = 0;
	...
	if(counter == 0 || counter == 2) {
		...
		do{
		}while(ev3_motor_get_counts(EV3_PORT_A) <= 245 * a);
...
void main_task(intptr_t unused) {
		...
		*pHT = 0;
		do{
			...
			if((*pHT == 1 || *pHT == 7 || *pHT == 8 || *pHT == 9 || *pHT == 10) && ship[0] == 0 && twice == 0) {
				...
				j += 1;
				twice = 1;
			}else if((*pHT == 2 || *pHT == 11) && ship[3] == 0 && twice == 0) {
				ev3_speaker_play_tone(261.63, 200);
				i = 3;
				ship2[j] = 4;
				j += 1;
	...
	if(1250 * a - angle >= 350 * a) {
		...
		do{
		}while(ev3_motor_get_counts(EV3_PORT_A) * -1 >= (1250 * a - angle - 250 * a) * -1);
	}

	ev3_motor_set_power(EV3_PORT_A, slow * -1);
	ev3_motor_set_power(EV3_PORT_D, slow);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) * -1 >= (1250 * a - angle) * -1);
	...
	ev3_motor_reset_counts(EV3_PORT_A);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) <= 420 * a);
	...
	if(container[0] != 0) {
		...
		if(container[0] == 3 || container[0] == 4) {
			...
			do{
			}while(ev3_motor_get_counts(EV3_PORT_B) >= -250);
			...
			ev3_motor_rotate(EV3_PORT_B, 420, 80, false);

			//速く移動
			ev3_motor_reset_counts(EV3_PORT_A);
			ev3_motor_reset_counts(EV3_PORT_D);
			do{
			}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -60 * a);
			...
			ev3_motor_reset_counts(EV3_PORT_A);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) <= 130 * a);
			...
			do{
			}while(ev3_motor_get_counts(EV3_PORT_B) >= -230);
			...
			ev3_motor_reset_counts(EV3_PORT_D);
			do{
			}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -180 * a);
			...
			ev3_motor_reset_counts(EV3_PORT_A);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) <= 270 * a);
		...
		if(container[3] == 3 || container[3] == 4) {
			...
			ev3_motor_reset_counts(EV3_PORT_D);
			do{
			}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -80 * a);
	...
	do{
		...
		*pHT = 0;
		...
		if (ship[container[counter_a] - 1] == ship[i]) {
			...
			if(ship[i] == 1){
				...
				ev3_motor_set_power(EV3_PORT_D, slow);
				do{
				}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) * -1) / 2 <= 60);

			}else{
				angle2 = (ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) * -1) / 2 + 95;

				ev3_motor_reset_counts(EV3_PORT_A);
				ev3_motor_reset_counts(EV3_PORT_D);
				do{
				}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) * -1) / 2 <= 95);
		...
		if (k == 0) {
			...
			if (container[1] == 0) {
				...
				if(angle >= 350 * a) {
					...
					do{
					}while(ev3_motor_get_counts(EV3_PORT_A) * -1 <= (angle - 250 * a));
				}
				...
				ev3_motor_set_power(EV3_PORT_D, -10);
				do{
				}while(ev3_motor_get_counts(EV3_PORT_A) * -1 >= anfw * a);
			...
			if (counter_c == 1 || counter_c == 0) {
				...
				if (counter_b == 1) {
					...
					if(1250 * a - angle >= 350 * a) {
						...
						ev3_motor_set_power(EV3_PORT_D, fast);
						do{
						}while(ev3_motor_get_counts(EV3_PORT_A) <= (1250 * a - angle - 250 * a));
					}

					ev3_motor_set_power(EV3_PORT_A, slow * -1);
					ev3_motor_set_power(EV3_PORT_D, slow);
					do{
					}while(ev3_motor_get_counts(EV3_PORT_A) <= (1250 * a - angle));
					...
					if(angle >= 350 * a) {
						...
						ev3_motor_set_power(EV3_PORT_D, fast);
						do{
						}while(ev3_motor_get_counts(EV3_PORT_A) <= (angle - 250 * a));
					}
				...
				if (counter_b == 1) {
					...
					if(1250 * a - angle >= 350 * a) {
						...
						ev3_motor_set_power(EV3_PORT_D, fast * -1);
						do{
						}while(ev3_motor_get_counts(EV3_PORT_A) * -1 <= (1250 * a - angle - 250 * a));
					}

					ev3_motor_set_power(EV3_PORT_A, slow);
					ev3_motor_set_power(EV3_PORT_D, slow * -1);
					do{
					}while(ev3_motor_get_counts(EV3_PORT_A) * -1 <= (1250 * a - angle));
		...
		switch (counter_c) {
			...
			case 1:
				...
				if(container[1] == 1 || container[1] == 2) {
					...
					ev3_motor_set_power(EV3_PORT_D, fast);
					do{
					}while(ev3_color_sensor_get_reflect(EV3_PORT_3) <= white || ev3_color_sensor_get_reflect(EV3_PORT_2) <= white);
					ev3_speaker_play_tone(1174.66, 200);
			...
			case 2:
				...
				if(container[2] == 3 || container[2] == 4) {
					...
					ev3_motor_set_power(EV3_PORT_D, fast);
					do{
					}while(ev3_color_sensor_get_reflect(EV3_PORT_2) <= white || ev3_color_sensor_get_reflect(EV3_PORT_3) <= white);
					ev3_speaker_play_tone(1174.66, 200);
	...
	//ゴールエリアにinする
	ev3_motor_rotate(EV3_PORT_A, 235 * a, slow * -1, false);
	ev3_motor_rotate(EV3_PORT_D, -235 * a, slow * -1, true);
