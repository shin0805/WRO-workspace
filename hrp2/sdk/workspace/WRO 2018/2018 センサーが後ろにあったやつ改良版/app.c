...
void change_yb_task(intptr_t unused) {
	...
	if(container[counter] == 4){
		...
		ev3_motor_reset_counts(EV3_PORT_A);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_A) >= -420);
...
void ship_left_task(intptr_t unused) {
	...
	angle = 60 + (ship[container[counter] - 1] - 1) * 270;
	...
	ev3_motor_rotate(EV3_PORT_A, -690 + angle, fast * -1, false);
	ev3_motor_rotate(EV3_PORT_D, -690 + angle, fast * -1, true);
...
void ship_right_task(intptr_t unused) {
	...
	angle = 60 + (6 - ship[container[counter] - 1]) * 270;
  ...
  //L字路まで進む
	...
	*pHT = 0;
	...
	if (container[1] != 0) {
		...
		ev3_motor_set_power(EV3_PORT_D, 30);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_A) <= 81);
