...
void change_yb_task(intptr_t unused) {
	...
	if(container[counter] == 4){
		...
		ev3_motor_reset_counts(EV3_PORT_A);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_A) >= -425 * a);
	...
	ev3_motor_rotate(EV3_PORT_A, -180 * a, 20, false);
	ev3_motor_rotate(EV3_PORT_D, -180 * a, 20, true);
	...
	ev3_motor_rotate(EV3_PORT_A, 79 * a, 30, false);
	ev3_motor_rotate(EV3_PORT_D, 79 * a, 30, true);
...
void change_rg_task(intptr_t unused) {
	...
	ev3_motor_set_power(EV3_PORT_D, 30);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_D) <= 60 * a);
...
void ship_left_task(intptr_t unused) {
	...
	angle = 60 + (ship[container[counter] - 1] - 1) * 266 * a;
...
void ship_right_task(intptr_t unused) {
	...
	angle = 60 + (6 - ship[container[counter] - 1]) * 266 * a;
...
void catch_task(intptr_t unused) {
	if(counter == 1) {
		...
		ev3_motor_set_power(EV3_PORT_D, 0);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_A) <= 215 * a);
		...
		do{
		}while(ev3_motor_get_counts(EV3_PORT_D) <= 300 * a);
		...
		ev3_motor_set_power(EV3_PORT_D, 30);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_D) <= 215 * a);
		...
		ev3_motor_rotate(EV3_PORT_A, -100 * a, 30, false);
		ev3_motor_rotate(EV3_PORT_D, 100 * a, 30, true);
	...
	ev3_motor_rotate(EV3_PORT_B, -280, 80, true);
...
void main_task(intptr_t unused) {
	...
	ev3_sta_cyc(TRACE22);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) >= -570 * a);
	...
	ev3_motor_reset_counts(EV3_PORT_A);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) >= -684 * a);
	...
	if(container[0] != 0) {
		...
		if(container[0] == 3 || container[0] == 4) {
			...
			ev3_motor_reset_counts(EV3_PORT_A);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) >= -700 * a);
	...
	ev3_motor_set_power(EV3_PORT_D, slow);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) >= -83 * a);
	...
	for (i = 1; i <= 5; i++) {
		...
		*pHT = 0;
		...
		if (ship[container[counter_a] - 1] == i) {
			...
			ev3_motor_rotate(EV3_PORT_A, 40 * a, 30, false);
			ev3_motor_rotate(EV3_PORT_D, 40 * a, 30, true);
			...
			//方向転換
			ev3_motor_rotate(EV3_PORT_A, 350 * a, 30, true);
		...
		if (i == 2) {
			...
			ev3_sta_cyc(TRACE2);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) >= -124 * a);
			ev3_stp_cyc(TRACE2);
		}
	...
	*pHT = 0;
	...
	if (ship[container[counter_a] - 1] == 6) {
		...
		//進む
		ev3_motor_rotate(EV3_PORT_A, -110 * a, 30, false);
		ev3_motor_rotate(EV3_PORT_D, -110 * a, 30, true);
	...
	for (k = 0; k < 2 ; k++) {
		...
		if (k == 0) {
			...
			if (container[1] == 0) {
				...
				//壁合わせ and 方向転換　(左へ)
				ev3_motor_reset_counts(EV3_PORT_A);
				ev3_motor_set_power(EV3_PORT_A, fast * -1 - 20);
				ev3_motor_set_power(EV3_PORT_D, fast * -1);
				do{
				}while(ev3_motor_get_counts(EV3_PORT_A) <= 1250 * a);
			...
			if (counter_c == 1) {
				...
				if (counter_b == 1) {
					...
					//少し下がる
					ev3_motor_rotate(EV3_PORT_A, 104 * a, 30, false);
					ev3_motor_rotate(EV3_PORT_D, 104 * a, 30, true);
		...
		switch (counter_c) {
			case 1:
				...
				if(container[1] == 1 || container[1] == 2) {
					...
					if(container[2] == 0){
						...
						ev3_motor_set_power(EV3_PORT_D, fast);
						do{
						}while(ev3_motor_get_counts(EV3_PORT_A) >= -415 * a);
			...
			case 3:
				...
				ev3_motor_reset_counts(EV3_PORT_A);
				do{
				}while(ev3_motor_get_counts(EV3_PORT_A) >= -352 * a);
	...
	if (counter_b == 1) {
		if (counter_d <= 3) {
			...
			ev3_motor_reset_counts(EV3_PORT_A);
			if(1350 * a - angle >= 100 * a) {
				ev3_motor_set_power(EV3_PORT_A, fast);
				ev3_motor_set_power(EV3_PORT_D, fast);
				do{
				}while(ev3_motor_get_counts(EV3_PORT_A) >= (1350 * a - angle - 100 * a) * -1);
			}
			ev3_motor_set_power(EV3_PORT_A, slow);
			ev3_motor_set_power(EV3_PORT_D, slow);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) >= (1350 * a - angle) * -1);
	...
	ev3_motor_reset_counts(EV3_PORT_A);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) >= -276 * a);

	ev3_stp_cyc(TRACE);

	//ゴールエリアにinする
	ev3_motor_rotate(EV3_PORT_A, -300 * a, slow * -1, false);
	ev3_motor_rotate(EV3_PORT_D, -300 * a, slow * -1, true);
