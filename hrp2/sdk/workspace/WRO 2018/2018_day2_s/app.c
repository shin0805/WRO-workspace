...
//gainの変数
double two_fast = 3.2;//2.14
double two_slow = 0.9;//2.14
double one_fast = 3.6;//4.0
double one_slow = 1.3;//1.9
...
void change_yb_task(intptr_t unused) {
	...
	if(container[counter] == 4){
		...
		do{
		}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -300 * a);//200

		power = slow;
		do{
		}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -425 * a);//390//420
	...
	ev3_motor_rotate(EV3_PORT_A, 165 * a, slow * -1, false);//175
	ev3_motor_rotate(EV3_PORT_D, -165 * a, slow * -1, true);
	...
	ev3_motor_rotate(EV3_PORT_A, -50 * a, 10, false);//55//60
	ev3_motor_rotate(EV3_PORT_D, 50 * a, 10, true);//55//60
	...
	ev3_motor_rotate(EV3_PORT_B, 150, 30, false);
...
void change_rg_task(intptr_t unused) {
	...
	ev3_motor_set_power(EV3_PORT_B, -25);
	ev3_motor_rotate(EV3_PORT_A, 165 * a, slow * -1, false);//170
	ev3_motor_rotate(EV3_PORT_D, -165 * a, slow * -1, true);//170
...
void ship_left_task(intptr_t unused) {
	...
	angle = 55 + (counter_d - 1) * 245 * a;//60//246
...
void ship_right_task(intptr_t unused) {
	...
	angle = 55 + (6 - counter_d) * 245 * a;//60//246
...
void catch_task(intptr_t unused) {
	switch (counter) {
		case 0:
			if (counter5 == 0){
				...
				ev3_motor_set_power(EV3_PORT_D, 30);
				do{
				}while(ev3_motor_get_counts(EV3_PORT_A) >= -140 * a);//125//135
				...
				power = slow;
				do{
				}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -420 * a);//430
				ev3_stp_cyc(TRACE);
		...
		case 1:
			...
			ev3_motor_set_power(EV3_PORT_D, 30);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) >= -50 * a);//25
		...
		case 3:
			...
			if (counter5 == 0){
				...
				ev3_motor_reset_counts(EV3_PORT_D);
				do{
				}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -210 * a);//200
				ev3_stp_cyc(TRACE);
			}else{
				power = slows;
				ev3_motor_reset_counts(EV3_PORT_A);
	...
	ev3_motor_rotate(EV3_PORT_B, -275, 80, true);//285//280
...
void curve_task(intptr_t unused) {
	switch (curve) {
		...
		case 4://内側への左
			...
			ev3_motor_set_power(EV3_PORT_D, 30 * a);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) <= 90 * a);//80
		...
		case 5://壁による方向転換 右
			...
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) >= -250 * a);//250

			ev3_motor_reset_counts(EV3_PORT_A);
			ev3_motor_set_power(EV3_PORT_A, slows);
			ev3_motor_set_power(EV3_PORT_D, slows);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) >= -65 * a);//60
		...
		case 6://壁による方向転換 左
			...
			ev3_motor_set_power(EV3_PORT_D, slows * -1);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_D) <= 65 * a);//70
...
void main_task(intptr_t unused) {
	...
	while(1){
		...
		sprintf(color_buffer2, "NUMBER-%e-" ,gain_rate);
	...
	if(container[0] != 0) {
		...
		if(container[0] == 3 || container[0] == 4) {
			ev3_motor_rotate(EV3_PORT_A, -95 * a, slow * -1, false);
			ev3_motor_rotate(EV3_PORT_D, 95 * a, slow * -1, true);
			...
			ev3_motor_reset_counts(EV3_PORT_D);
			do{
			}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -90 * a);
			...
			ev3_motor_reset_counts(EV3_PORT_D);
			do{
			}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -20 * a);
			...
			ev3_motor_rotate(EV3_PORT_B, 360, 80, false);

			//速く移動
			ev3_motor_reset_counts(EV3_PORT_A);
			ev3_motor_reset_counts(EV3_PORT_D);
			do{
			}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -40 * a);//250
			...
			ev3_motor_reset_counts(EV3_PORT_D);
			do{
			}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -340 * a);//550
		...
		if(container[3] == 3 || container[3] == 4) {
			...
			if(yes_container[0] == 0){
				...
				ev3_motor_reset_counts(EV3_PORT_D);
				do{
				}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -90 * a);//300
			...
			ev3_motor_reset_counts(EV3_PORT_D);
			do{
			}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -140 * a);//350
		...
		*pHT = 0;
		...
		if (ship[container[counter_a] - 1] == counter_d) {
			...
			if(counter_d == 1){
				...
				ev3_motor_set_power(EV3_PORT_D, -10);
				do{
				}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) * -1) / 2 <= 55);//60

				//turn = turn - 2;
				//counter_e = 2;

			}else if(counter_d == 6){
				angle2 = (ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) * -1) / 2 + 85;//105//90//85

				ev3_motor_reset_counts(EV3_PORT_A);
				ev3_motor_reset_counts(EV3_PORT_D);
				do{
				}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) * -1) / 2 <= 85);//105//90//85
				...
				angle2 = (ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) * -1) / 2 + 115;//105//90
			...
			if(ship2[2] == 0) {
				...
				if(counter_d == 1){
					...
					do{
					}while(ev3_motor_get_counts(EV3_PORT_D) >= -155 * a);

					//アームを元に戻す
					ev3_motor_set_power(EV3_PORT_C, 80);

					ev3_motor_reset_counts(EV3_PORT_A);
					ev3_motor_set_power(EV3_PORT_A, slow * -1 - 5);//-6
					ev3_motor_set_power(EV3_PORT_D, slow);
					...
					ev3_motor_reset_counts(EV3_PORT_A);
					do{
					}while(ev3_motor_get_counts(EV3_PORT_A) <= 190 * a);//185
	...
	for (k = 0; k < 3 ; k++) {
		...
		if (k == 0 && counter_e == 0) {
			...
			if (no_container != 0 && no_container != 1) {
				if(counter_s == 1){
					counter_c = no_container;
				}else{
					counter_c = 2;
				}
				...
				ev3_motor_set_power(EV3_PORT_D, -10);
				do{
				}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) + -1) / 2 >= -40 + 5 * a);
			...
			if(counter_s != 2){
				...
				if (counter_c == 0 || counter_c == 1) {
					...
					if (counter_d == 6){
						...
						ev3_motor_set_power(EV3_PORT_D, -20);
						do{
						}while(ev3_motor_get_counts(EV3_PORT_A) <= left_curve * a);//540
					...
					if (counter_d == 1){
						...
						ev3_motor_set_power(EV3_PORT_D, slow - 20);
						do{
						}while(ev3_motor_get_counts(EV3_PORT_D) >= -170 * a);//180
						...
						ev3_motor_set_power(EV3_PORT_D, -60);
						do{
						}while(ev3_motor_get_counts(EV3_PORT_D) >= right_curve * -1 * a);//540
		...
		switch (counter_c) {
			case 0:
				...
				if(container[0] == 1 || container[0] == 2) {
					...
					ev3_motor_reset_counts(EV3_PORT_D);
					do{
					}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= 0 * a);//300
			...
			case 1:
				...
				if(container[1] == 1 || container[1] == 2) {
					...
					if(yes_container[3] == 0){
						...
						ev3_motor_set_power(EV3_PORT_D, fast);
						do{
						}while(ev3_motor_get_counts(EV3_PORT_D) >= -210 * a);//200
						...
						ev3_motor_reset_counts(EV3_PORT_D);
						do{
						}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -50 * a);//90//70
						...
						ev3_motor_reset_counts(EV3_PORT_D);
						do{
						}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= 0 * a);//200
						...
						ev3_motor_set_power(EV3_PORT_D, slows);
						do{
						}while(ev3_motor_get_counts(EV3_PORT_D) >= -190 * a);
						...
						ev3_motor_reset_counts(EV3_PORT_D);
						do{
						}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -230 * a);//350
			...
			case 2:
				...
				if(container[2] == 3 || container[2] == 4) {
					...
					if(yes_container[0] == 0){
						...
						ev3_motor_set_power(EV3_PORT_D, 0);
						do{
						}while(ev3_motor_get_counts(EV3_PORT_A) <= 210 * a);//180
						...
						ev3_motor_reset_counts(EV3_PORT_D);
						do{
						}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -70 * a);//90
		...
		if(counter_s == 2){
			yes_container[no_container] = 0;
			container[no_container] = 0;
			counter_s2 = 1;
	...
	if (counter_d >= 4) {
		...
		if (counter_d == 6){
			...
			ev3_motor_set_power(EV3_PORT_D, -20);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) <= left_curve * a);
		...
		if (counter_d == 1){
			...
			ev3_motor_set_power(EV3_PORT_D, -60);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_D) >= right_curve * -1 * a);
	...
	ev3_motor_reset_counts(EV3_PORT_A);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) <= 110 * a);
	...
	ev3_motor_reset_counts(EV3_PORT_D);
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) * -1) / 2 <= 250 * a);
