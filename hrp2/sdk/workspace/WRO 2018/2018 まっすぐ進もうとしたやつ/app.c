...
void change_yb_task(intptr_t unused) {
	...
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) >= -95 * a);
	...
	ev3_sta_cyc(STRAIGHT);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) >= -45 * a);
	ev3_stp_cyc(STRAIGHT);
...
void ship_left_task(intptr_t unused) {
	...
	ev3_sta_cyc(STRAIGHT);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) <= 65 * a);
	ev3_stp_cyc(STRAIGHT);
...
void straight_task(intptr_t unused) {
	kp = 0.36 * gain_rate;
	ki = kp / 0.3;
	kd = kp * 0.075;

	diff[0] = diff[1];
	sensor2 = ev3_motor_get_counts(EV3_PORT_A);
	sensor3 = ev3_motor_get_counts(EV3_PORT_D);
...
void main_task(intptr_t unused) {
	...
	if(container[0] != 0) {
		...
		if(container[3] == 3 || container[3] == 4) {
			...
			ev3_motor_reset_counts(EV3_PORT_A);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) >= -175 * a);
			ev3_stp_cyc(STRAIGHT);
	...
	ev3_motor_reset_counts(EV3_PORT_A);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) <= 235 * a);
	ev3_stp_cyc(STRAIGHT);
