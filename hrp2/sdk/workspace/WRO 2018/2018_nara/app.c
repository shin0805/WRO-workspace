...
void change_yb_task(intptr_t unused) {
	...
	if(container[counter] == 4){
		...
		ev3_motor_reset_counts(EV3_PORT_D);
		do{
		}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -370 * a);
		...
		do{
		}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 20 * a);
	}
	...
	ev3_motor_rotate(EV3_PORT_A, 60 * a, 30, false);
	ev3_motor_rotate(EV3_PORT_D, 60 * a, 30, true);
	...
	if(port == 1){
		...
		*pHT = 0;
		...
		if(separate == 1 && container[counter] == 0){
			container[counter] = 4;
			ev3_speaker_play_tone(261.63, 200);
		}else if(separate == 2 && container[counter] == 0){
			container[counter] = 3;
			ev3_speaker_play_tone(880.00, 200);
		}
...
void ship_left_task(intptr_t unused) {
	...
	angle = 43 + (ship[container[counter] - 1] - 1) * 250 * a;

	if(ship[container[counter] - 1] >= 3) {
		Tt = angle;
		countt = 0;
		act_tsk(TRAPEZOIDAL_TASK);
		slp_tsk();
	}else{
		ev3_motor_reset_counts(EV3_PORT_D);
		ev3_motor_set_power(EV3_PORT_A, slow);
...
void ship_right_task(intptr_t unused) {
	...
	angle = 43 + (6 - ship[container[counter] - 1]) * 250 * a;

	if(ship[container[counter] - 1] <= 4) {
		Tt = angle;
		countt = 1;
		act_tsk(TRAPEZOIDAL_TASK);
		slp_tsk();
...
void catch_task(intptr_t unused) {
	switch (counter) {
		...
		case 2:
			...
			ev3_motor_set_power(EV3_PORT_D, 30);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_D) <= 55 * a);
		...
		case 3:
			...
			if (counter5 == 0){
				...
				ev3_motor_reset_counts(EV3_PORT_D);
				do{
				}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -180 * a);
				ev3_stp_cyc(TRACE);
			}else{
				ev3_motor_reset_counts(EV3_PORT_A);
				ev3_motor_reset_counts(EV3_PORT_D);
				do{
				}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -325 * a);
				ev3_stp_cyc(TRACE);
			}
			counter5 = 0;
	...
	ev3_motor_rotate(EV3_PORT_B, -295, 80, true);
...
void main_task(intptr_t unused) {
	...
	if(container[0] != 0) {
		...
		if(container[0] == 3 || container[0] == 4) {
			...
			ev3_motor_reset_counts(EV3_PORT_D);
			do{
			}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -80 * a);
			...
			ev3_motor_reset_counts(EV3_PORT_D);
			do{
			}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -200 * a);
		...
		//軸に乗せる
		ev3_motor_rotate(EV3_PORT_A, -90 * a, 20, false);
		ev3_motor_rotate(EV3_PORT_D, -90 * a, 20, true);
		...
		if(container[3] == 3 || container[3] == 4) {
			...
			ev3_motor_set_power(EV3_PORT_D, fast);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) >= -740 * a);
			...
			//contaienr[3]が赤、緑の場合
			ev3_motor_rotate(EV3_PORT_A, 190 * a, 30, false);
			ev3_motor_rotate(EV3_PORT_D, 190 * a, 30, true);
			...
			ev3_motor_reset_counts(EV3_PORT_D);
			do{
			}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -100 * a);
	...
	for (i = 1; i <= 5; i++) {
		...
		*pHT = 0;
		ev3_sta_cyc(HT4);
		do{
			if((*pHT == 7 || *pHT == 8 || *pHT == 9 || *pHT == 10) && ship[0] == 0 && twice == 0) {
				ev3_speaker_play_tone(1975.53, 200);
				ship[0] = i;
				ship2[j] = 1;
				j += 1;
				...
				twice = 1;
				separate = 0;
			}else if((*pHT == 2/* || *pHT == 3*/) && ship[3] == 0 && twice == 0) {
				ev3_speaker_play_tone(261.63, 200);
				ship[3] = i;
				ship2[j] = 4;
				j += 1;
				twice = 1;
				separate = 0;
			}else if(*pHT == 3 && twice == 0) {
				separate = 1;
			}else if(*pHT == 5 && twice == 0) {
				separate = 2;
			}else{
		...
		if (ship[container[counter_a] - 1] == i) {
			...
			ev3_motor_rotate(EV3_PORT_D, 120 * a, 40, true);
			ev3_motor_stop(EV3_PORT_D, true);

			ev3_motor_rotate(EV3_PORT_A, 120 * a, 40, true);
	...
	for (k = 0; k < 2 ; k++) {
		...
		if (k == 0) {
			...
			if (container[1] == 0) {
				...
				//少し下がる
				ev3_motor_reset_counts(EV3_PORT_A);
				ev3_motor_steer(EV3_PORT_A, EV3_PORT_D, -20, 0);
				do{
			...
			if (counter_c == 1) {
				...
				if (counter_b == 1) {
					...
					if(angle >= 100 * a) {
						...
						ev3_motor_set_power(EV3_PORT_D, fast);
						do{
						}while(ev3_motor_get_counts(EV3_PORT_A) >= (angle - 100 * a) * -1);
					}
					ev3_motor_set_power(EV3_PORT_A, slow);
					ev3_motor_set_power(EV3_PORT_D, slow);
					do{
					}while(ev3_motor_get_counts(EV3_PORT_A) >= angle * -1);
				...
				if (counter_b == 1) {
					...
					if(1250 * a - angle >= 100 * a) {
						...
						ev3_motor_set_power(EV3_PORT_D, fast * -1);
						do{
						}while(ev3_motor_get_counts(EV3_PORT_A) <= (1250 * a - angle - 100 * a));
					}
					ev3_motor_set_power(EV3_PORT_A, slow * -1);
					ev3_motor_set_power(EV3_PORT_D, slow * -1);
					do{
	...
	if (counter_b == 1) {
		...
		if (counter_d <= 3) {
			...
			if(angle >= 300 * a) {
				...
				ev3_motor_set_power(EV3_PORT_D, fast);
				do{
				}while(ev3_motor_get_counts(EV3_PORT_A) >= (angle - 300 * a) * -1);
			}
