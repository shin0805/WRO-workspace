...
void main_task(intptr_t unused) {
	...
	while(1){
		...
		ev3_motor_rotate(EV3_PORT_C, 120 * a, 20, true);
		...
		*pHT = 0;
		...
		if (ship[container[counter_a] - 1] == counter_d) {
			...
			if(counter_d == 1){
				...
				ev3_motor_set_power(EV3_PORT_D, -10);
				do{
				}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) * -1) / 2 <= 60);//55
  ...
  do{
	...
	do{
		...
		angle = 60 + (counter_d - 1) * 246 * a - angle2;//55
		...
		if(ev3_color_sensor_get_reflect(EV3_PORT_2) >= threshold || ev3_color_sensor_get_reflect(EV3_PORT_3) >= threshold){
			break;
		}
		...
		ev3_motor_set_power(EV3_PORT_D, slow);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_A) <= 510 * a);
	...
	ev3_motor_rotate(EV3_PORT_C, 125 * a, 20, true);
	...
	ev3_motor_set_power(EV3_PORT_D, -30);
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) + -1) / 2 >= (right * 2 - 80) * -1 * a);
	...
	ev3_motor_rotate(EV3_PORT_B, 330, 80, true);
	...
	ev3_motor_set_power(EV3_PORT_D, slow * -1);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) >= -340 * a);
