...
void ship_left_task(intptr_t unused) {
	...
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) + -1) / 2 >= -50 * a);
	...
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) + -1) / 2 >= (turn - 90) * -1 * a);
...
void ship_right_task(intptr_t unused) {
	...
	ev3_motor_set_power(EV3_PORT_D, 40);
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) + -1) / 2 <= turn - 90 * a);
...
void main_task(intptr_t unused) {
	...
	while(1){
		...
		ev3_motor_set_power(EV3_PORT_D, slow * -1);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_D) <= 65 * a);
	...
	for (k = 0; k < 2 ; k++) {
		...
		if (k == 0 && counter_e == 0) {
			...
			if (container[1] == 0) {
				...
				ev3_motor_set_power(EV3_PORT_D, -40);
				do{
				}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) + -1) / 2 >= (turn - 85) * -1 * a);
			...
			if (counter_c == 0 || counter_c == 1) {
				...
				if (counter_d == 1){
					...
					ev3_motor_set_power(EV3_PORT_D, 40);
					do{
					}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) + -1) / 2 <= turn - 85 * a);
