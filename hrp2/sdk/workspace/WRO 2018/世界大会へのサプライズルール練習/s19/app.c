...
void change_yb_task(intptr_t unused) {
	...
	if(container[counter] == 4){
		...
		power = slow;
		do{
		}while((ev3_motor_get_counts(EV3_PORT_A) * -1 + ev3_motor_get_counts(EV3_PORT_D)) / 2 >= -420 * a);//390
	...
	ev3_motor_rotate(EV3_PORT_A, -60 * a, 10, false);//55
	ev3_motor_rotate(EV3_PORT_D, 60 * a, 10, true);//55
...
void catch_task(intptr_t unused) {
	switch (counter) {
		...
		case 1:
			...
			ev3_motor_set_power(EV3_PORT_D, 30);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) >= -35 * a);//25
...
void main_task(intptr_t unused) {
		...
		*pHT = 0;
		...
		if (ship[container[counter_a] - 1] == counter_d) {
			...
			if(counter_d == 1){
				...
				angle2 = (ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) * -1) / 2 + 90;//105//90
	...
	if (counter_a == 1) {
		...
		if(no_container == 3){
			...
			ev3_motor_set_power(EV3_PORT_B, -25);
			ev3_motor_rotate(EV3_PORT_A, 100 * a, slow * -1, false);//175
			ev3_motor_rotate(EV3_PORT_D, -100 * a, slow * -1, true);
			...
			ev3_motor_rotate(EV3_PORT_A, -100 * a, 10, false);//55//60
			ev3_motor_rotate(EV3_PORT_D, 100 * a, 10, true);//55
			...
			ev3_motor_rotate(EV3_PORT_A, -200 * a, 10, false);//55//60
			ev3_motor_rotate(EV3_PORT_D, 200 * a, 10, true);//55
