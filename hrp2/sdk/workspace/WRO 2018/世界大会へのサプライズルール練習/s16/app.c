...
void main_task(intptr_t unused) {
	...
	while(1){
		...
		ev3_motor_set_power(EV3_PORT_C, 10);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_C) <= 70 * a);
	...
	container[no_container] = no_color;
	counter_s = 1;
	...
	if (no_color == 3 || no_color == 4) {
		...
		if (counter_d == 1){
			...
			ev3_motor_set_power(EV3_PORT_D, slow - 20);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_D) >= -170 * a);
	...
	if(no_container == 0){
		...
		ev3_motor_set_power(EV3_PORT_D, 30);
		do{
		}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) + -1) / 2 <= (left * 2 - 80) * a);
		...
		ev3_motor_rotate(EV3_PORT_A, -210 * a, slow * -1, false);
		ev3_motor_rotate(EV3_PORT_D, 210 * a, slow * -1, true);
		...
		ev3_motor_reset_counts(EV3_PORT_A);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_A) <= 290 * a);
