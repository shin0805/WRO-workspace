...
void main_task(intptr_t unused) {
	...
	do{
		...
		*pHT = 0;
		...
		if(counter_e == 0){
			ship[i] = counter_d;
		}else if(counter_e != 0 && counter_f == 1){
			counter_s1 = counter_d;
		}else if(counter_e != 0 && counter_f == 2){
			ev3_stp_cyc(TRACE33);
			...
			*pHT = 0;
			...
			if(*pHT != 0) {
				ev3_speaker_play_tone(3000, 200);
				counter_s2 = counter_s1;
				counter_s1 = counter_d;
			}else{
	...
	}while(ship2[2] == 0 || counter_s1 == 0 || counter_s2 == 0);
	...
	sprintf(color_buffer1, "Number:::%d" ,container[1]);
	...
	if (counter_s1 >= 4) {
		...
		if (counter_d == 6){
			...
			ev3_motor_set_power(EV3_PORT_D, -10);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) <= anbw * a);
