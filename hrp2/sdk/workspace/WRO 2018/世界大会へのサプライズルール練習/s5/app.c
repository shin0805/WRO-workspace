...
void main_task(intptr_t unused) {
	...
	//contianer[1]の特定
	container[1] = 10 - (container[0] + container[2] + container[3]);
	...
	for (k = 0; k < 3 ; k++) {
		...
		if (k == 0 && counter_e == 0) {
			...
			if (container[1] == 0) {
				...
				do{
				}while(ev3_motor_get_counts(EV3_PORT_D) <= 245 * (ship[ship2[3] - 1] - 2) - 25);
				...
				ev3_motor_set_power(EV3_PORT_D, fast);
				do{
				}while(ev3_motor_get_counts(EV3_PORT_A) <= 245 * (6 - ship[ship2[3] - 1]) - 45);
