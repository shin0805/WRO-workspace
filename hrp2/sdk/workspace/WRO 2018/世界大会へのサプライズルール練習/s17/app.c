...
int counter_s1;//サプライズの色

int counter_s2;//サプライズの船の位置

int counter_s3;//サプライズのコンテナの位置

int counter_s4;//サプライズか否か
...
void ship_left_task(intptr_t unused) {
	...
	if(counter_s4  == 1){
		...
		counter_e = ship[container[counter] - 1];
	}

	counter_d = counter_e;

	angle = 60 + (counter_d - 1) * 246 * a;//55
...
void curve_task(intptr_t unused) {
	switch (curve) {
		...
		case 5://壁による方向転換 右
			...
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) >= -260 * a);//250

			ev3_motor_reset_counts(EV3_PORT_A);
			ev3_motor_set_power(EV3_PORT_A, slows);
			ev3_motor_set_power(EV3_PORT_D, slows);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) >= -70 * a);//60
		...
		case 6://壁による方向転換 左
			...
			do{
			}while(ev3_motor_get_counts(EV3_PORT_D) <= 260 * a);//240

			ev3_motor_reset_counts(EV3_PORT_D);
			ev3_motor_set_power(EV3_PORT_A, slows * -1);
			ev3_motor_set_power(EV3_PORT_D, slows * -1);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_D) <= 70 * a);//70
...
void main_task(intptr_t unused) {
	...
	do{
		...
		*pHT = 0;
		do{
			...
			if(*pHT != 0 && *pHT <= 13) {
				tslp_tsk(20);
				if((*pHT == 1 || *pHT == 7 || *pHT == 8 || *pHT == 9 || *pHT == 10) && twice == 0) {
					ev3_speaker_play_tone(1975.53, 200);
					...
					j += 1;
					twice = 1;
					s_twice[0] = s_twice[0] + 1;
					if(s_twice[0] == 2){
						counter_e = 1;
					}
				}else if((*pHT == 4 || *pHT == 13 || *pHT == 12) && twice == 0) {
					ev3_speaker_play_tone(1396.91, 200);
					counter_s1 = counter_s1 - 2;
					i = 1;
					counter_s[j] = 2;
					j += 1;
					twice = 1;
					s_twice[1] = s_twice[1] + 1;
					if(s_twice[1] == 2){
						counter_e = 2;
					}
				}else if((*pHT == 5 || *pHT == 6) && twice == 0) {
					ev3_speaker_play_tone(880.00, 200);
					counter_s1 = counter_s1 - 3;
					i = 2;
					counter_s[j] = 3;
					j += 1;
					twice = 1;
					s_twice[2] = s_twice[2] + 1;
					if(s_twice[2] == 2){
						counter_e = 3;
					}
				}else if((*pHT == 2 || *pHT == 11) && twice == 0) {
					ev3_speaker_play_tone(261.63, 200);
					counter_s1 = counter_s1 - 4;
					i = 3;
					counter_s[j] = 4;
					j += 1;
					twice = 1;
					s_twice[3] = s_twice[3] + 1;
					if(s_twice[3] == 2){
						counter_e = 4;
					}
				}else{
		...
		if(counter6 == 0){
			angle = ev3_motor_get_counts(EV3_PORT_D) * -1;
		}else{
			angle = ev3_motor_get_counts(EV3_PORT_D) * -1 + angle2 ;
		}
		...
		if(counter_e != 0){
			...
			ev3_motor_set_power(EV3_PORT_A, slows);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) >= counter_f * -1);
			...
			*pHT = 0;
			...
			if(*pHT != 0 && *pHT <= 13) {
				ev3_speaker_play_tone(1975.53, 200);
				counter_s2 = counter_d;
				counter_s3 = ship[counter_e -1];
			}else{
				counter_s2 = ship[counter_e -1];
				counter_s3 = counter_d;
			...
			ev3_motor_set_power(EV3_PORT_A, slows * -1);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) <= counter_f);
		...
		if (j != 5){
			...
			ev3_motor_set_power(EV3_PORT_D, slows);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) <= 115);//105//90
		}
	...
	ev3_motor_set_power(EV3_PORT_D, fast);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) <= 245 * (6 - counter_d) - 45);
	...
	//contianer[1]の特定
	container[1] = (10 - counter_s1) - (container[0] + container[2] + container[3]);
	...
	sprintf(color_buffer1, "Number:::%d" ,counter_s3);
	...
	if (counter_s3 >= 4) {
		...
		if (counter_d == 6){
			...
			ev3_motor_reset_counts(EV3_PORT_A);
			ev3_motor_set_power(EV3_PORT_A, slow);
			ev3_motor_set_power(EV3_PORT_D, slow + 6);
			do{
			...
			ev3_motor_set_power(EV3_PORT_D, -20);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) <= 530 * a);
	...
	if (counter_s1 == 3 || counter_s1  == 4) {
		...
		if (counter_d == 6){
			...
			ev3_motor_set_power(EV3_PORT_D, 30);
			do{
			}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) + -1) / 2 <= (left * 2 - 80 + 15) * a);
			...
			ev3_motor_set_power(EV3_PORT_D, 60);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_D) <= 570 * a);//540//520

			ev3_motor_reset_counts(EV3_PORT_A);
			ev3_motor_set_power(EV3_PORT_A, fast);
			ev3_motor_set_power(EV3_PORT_D, fast * -1);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) * -1 <= 240 * (5 - counter_d) * a);
			...
			ev3_motor_set_power(EV3_PORT_D, -10);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) <= anfw * a);
		...
		if (counter_d == 1){
			...
			ev3_motor_set_power(EV3_PORT_D, -30);
			do{
			}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) + -1) / 2 >= (right * 2 - 80 + 15) * -1 * a);
			...
			ev3_motor_set_power(EV3_PORT_D, 20);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) >= -570 * a);//540//520

			ev3_motor_reset_counts(EV3_PORT_A);
			ev3_motor_set_power(EV3_PORT_A, fast);
			ev3_motor_set_power(EV3_PORT_D, fast * -1);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) * -1 <= 240 * (counter_d - 2) * a);
