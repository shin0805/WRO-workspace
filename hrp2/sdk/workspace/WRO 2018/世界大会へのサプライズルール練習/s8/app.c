...
void main_task(intptr_t unused) {
	...
	//軸に乗せる
	ev3_motor_rotate(EV3_PORT_B, -285, 80, false);

	ev3_motor_rotate(EV3_PORT_A, -70 * a, slow * -1, false);
	ev3_motor_rotate(EV3_PORT_D, 70 * a, slow * -1, true);
	...
	//方向転換
	ev3_motor_rotate(EV3_PORT_A, 365 * a, slow * -1, true);//365
	...
	if(counter_e == 1){
		...
		}while(ev3_motor_get_counts(EV3_PORT_A) <= 260);//275

		ev3_stp_cyc(TRACE22);
		ev3_stp_cyc(HT1);

		counter_s = ev3_motor_get_counts(EV3_PORT_D) * -1;
		ev3_motor_stop(EV3_PORT_A, true);
		ev3_motor_stop(EV3_PORT_D, true);
		tslp_tsk(300);
	...
	ev3_motor_reset_counts(EV3_PORT_A);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) <= counter_s * a);
		...
		*pHT = 0;
		...
		if (ship[container[counter_a] - 1] == counter_d) {
			...
			if(counter_d == 1){
				...
				angle2 = (ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) * -1) / 2 + 110;//105//90

				ev3_motor_reset_counts(EV3_PORT_A);
				ev3_motor_reset_counts(EV3_PORT_D);
				do{
				}while((ev3_motor_get_counts(EV3_PORT_A) + ev3_motor_get_counts(EV3_PORT_D) * -1) / 2 <= 110);//105//90
