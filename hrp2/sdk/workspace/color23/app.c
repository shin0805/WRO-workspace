...
void main_task(intptr_t unused)
{
  ...
  ev3_sensor_config(EV3_PORT_4, COLOR_SENSOR);	//左後HTセンサー
  ...
  ev3_motor_config(EV3_PORT_C, MEDIUM_MOTOR);	//下ろすやつ
    ...
    */
    ...
    while(1) {
        a = ev3_color_sensor_get_color(EV3_PORT_2);
        b = ev3_color_sensor_get_color(EV3_PORT_3);
        c = ev3_color_sensor_get_color(EV3_PORT_4);

        sprintf(color_buffer1, "NUMBER:2-%d-" ,a);
        sprintf(color_buffer2, "NUMBER:3-%d-" ,b);
        sprintf(color_buffer3, "NUMBER:4-%d-" ,c);


        /* 正面ディスプレイに文字配列の中身を描写させる */
        ev3_lcd_draw_string(color_buffer1, 0,10);
        ev3_lcd_draw_string(color_buffer2, 0,40);
        ev3_lcd_draw_string(color_buffer3, 0,70);
    }
    ext_tsk();
}
