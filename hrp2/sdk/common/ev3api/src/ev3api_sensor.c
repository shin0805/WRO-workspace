...
static inline int16_t analog_sensor_get_pin1(sensor_port_t port)
{
    return pAnalogSensorData[port].pin1[*pAnalogSensorData[port].actual];
}

static inline int16_t analog_sensor_get_pin6(sensor_port_t port)
{
 ...
 */
static void uart_sensor_fetch_data(sensor_port_t port, uint8_t mode, void* dest, SIZE size)
{
    ER ercd;
    ...
    while (!((*pUartSensorData[port].status) & UART_DATA_READY))
        ;  // TODO: time out
