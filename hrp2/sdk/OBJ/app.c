...
int black = 10;//20
int white = 45;//40
int threshold2sub = 25;//35
int threshold3sub = 35;//25
	...
	*/
	...
	switch (curve) {
		...
		case 7://真ん中への右 (両方黒から)
			...
			ev3_motor_set_power(EV3_PORT_D, -30);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_D) * -1 <= 155);//170
		...
		case 8://真ん中への左 (両方黒から)
			...
			ev3_motor_set_power(EV3_PORT_D, 0);
			do{
			}while(ev3_motor_get_counts(EV3_PORT_A) <= 155);//170
...
void ht4color_task(intptr_t unused) {
	...
	if(choice == 1){//赤
		if(rgb4.b > 0 && rgb4.b <= 220){//230//170//210//220//210
			wup_tsk(MAIN_TASK);
		}
	}else if(choice == 2){//緑
		if(rgb4.g > 0 && rgb4.g <= 220){//r//170//230//210//200
			wup_tsk(MAIN_TASK);
		}
	}else if(choice == 3){//黄
		if(rgb4.b > 0 && rgb4.b <= 220){//170//230//205//220
			wup_tsk(MAIN_TASK);
		}
	}else if(choice == 4){//青
		if(rgb4.g > 0 && rgb4.g <= 220){//170//200//210
			wup_tsk(MAIN_TASK);
		}
	}
...
void drop_task(intptr_t unused) {
	if(direction == 1){
		...
		*/
		...
		//ev3_motor_set_power(EV3_PORT_C, 20);
		ev3_motor_rotate(EV3_PORT_C, 200 + delta, 20, false);//300
		...
		if(counter == 0){
			...
			ev3_motor_reset_counts(EV3_PORT_A);
			ev3_motor_reset_counts(EV3_PORT_D);
			ev3_motor_set_power(EV3_PORT_A, power + 2);
		...
		*/
		...
		//ev3_motor_set_power(EV3_PORT_C, 20);
		ev3_motor_rotate(EV3_PORT_C, 305 + delta, 20, false);
		/*
		do{
		}while(ev3_motor_get_counts(EV3_PORT_C) <= 615);
		...
		*/
		...
		//ev3_motor_set_power(EV3_PORT_C, 20);
		ev3_motor_rotate(EV3_PORT_C, 200 + delta, 20, false);//300//200
		...
		*/
		...
		ev3_motor_reset_counts(EV3_PORT_A);
		ev3_motor_reset_counts(EV3_PORT_D);
		ev3_motor_set_power(EV3_PORT_A, power * -1 + 2);
		ev3_motor_set_power(EV3_PORT_D, power);
		do{
		...
		//リンクをたたみきる
		ev3_motor_rotate(EV3_PORT_C, 275 + delta, 20, false);
		//ev3_motor_set_power(EV3_PORT_C, 20);
		/*
		do{
		}while(ev3_motor_get_counts(EV3_PORT_C) <= 615);
...
void main_task(intptr_t unused) {
	...
	*/
	...
	while(1){
		...
		ev3_motor_rotate(EV3_PORT_C, -135, 30, false);
	...
	ev3_motor_rotate(EV3_PORT_C, -140, 30, false);//190//160//170//180が落ち着いた頃//185は満タン時//135//120
	...
	ev3_motor_reset_counts(EV3_PORT_D);
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 55);
	...
	ev3_motor_set_power(EV3_PORT_D, 20);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_A) <= 260);//
	...
	*/
	...
	ev3_sta_cyc(BACK);
	do{
	}while((ev3_motor_get_counts(EV3_PORT_D) - ev3_motor_get_counts(EV3_PORT_A)) / 2 <= 190);
	ev3_stp_cyc(BACK);
	...
	*/
	...
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 480);//600//540
	...
	*/
	...
	if(device[0] == 2){
		...
		*/
		...
		ev3_motor_set_power(EV3_PORT_D, slow * -1);
		do{
		}while(ev3_motor_get_counts(EV3_PORT_D) <= 520);//515
	...
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 470);//600//400//340//510
	...
	*/
	...
	ev3_motor_set_power(EV3_PORT_D, -30);
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 145);//125//150//145//140
	...
	*/
	...
	ev3_sta_cyc(BACK);
	do{
	}while((ev3_motor_get_counts(EV3_PORT_D) - ev3_motor_get_counts(EV3_PORT_A)) / 2 <= 180);//150//200
	ev3_stp_cyc(BACK);
	...
	*/
	...
	ev3_motor_reset_counts(EV3_PORT_D);
	do{
	}while((ev3_motor_get_counts(EV3_PORT_A) - ev3_motor_get_counts(EV3_PORT_D)) / 2 <= 1650);//1950
	...
	*/
	...
	ev3_motor_set_power(EV3_PORT_D, slow);
	do{
	}while(ev3_motor_get_counts(EV3_PORT_D) * -1 <= 495);
